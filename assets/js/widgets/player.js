;(function($, undefined) {
    var play_videodefaults = {};
    var play_video = function(element,options){

        this.$el = $(element);
        this.options = options;
        this.$container = $(this.options.id);
        this.options = $.extend({}, play_videodefaults, options);
        this.initialized();
    };
    
    play_video.prototype = {
        ...
        events : {
            'click [node-type="dataedit"]':'user_edit',
            'click [node-type="save_edit"]':'edit_user',// 保存按钮
            'click [node-type="close"]':'hide',
            'click [node-type="reset"]':'no'
            // 'click [node-type="sextype"]' : 'radio_event'
            
        },

        
    };
    $.fn.play_video = function (option) {
        // console.log(123);
       
      return this.each(function(input_field) {
            
      var $this = $(this)
            , data = $this.data('play_video')
            , options = typeof option == 'object' && option
            if (!data) {
                $this.data('play_video', (data = new play_video(this, options)));
            }
            else if(!!options){
                data['reset'](options);
            }
  ...
  var video = $('#video')[0];
  video.volume = 0.05;
  var playurl = ' http://123.56.11.45:80/hls/test2.m3u8';
  // playurl = 'http://gfchek8d6spzdv02n01.exp.bcelive.com/lss-gfch54a0ydcywkw1/live.m3u8';
  // playurl = 'http://demo-video.bj.bcebos.com/AdrenalineRush.m3u8';
  //playurl = 'http://www.streambox.fr/playlists/x36xhzz/x36xhzz.m3u8'
  loadStream(decodeURIComponent(getURLParam('src',playurl)));
